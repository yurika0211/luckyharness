...
### v0.84.0: config 包补测到 80% (78.7%→80%+)

**状态**: Ready
**优先级**: High
**目标**: config 包 78.7% → 80%+ (+1.3pp)
**预计耗时**: 10-15 分钟
**关键点**: mock `os.UserHomeDir()` 覆盖 `NewManager` 剩余 25%

**待办**:
- [ ] 分析 config 包剩余未覆盖行
- [ ] 使用 `t.Setenv("HOME", "/tmp")` mock 用户目录
- [ ] 补充 `NewManager` 完整路径测试
- [ ] 运行测试验证覆盖率 → 目标 80%+
- [ ] 提交并推送代码

**备注**: config 包当前 78.7%，距离 80% 目标还差 1.3pp
