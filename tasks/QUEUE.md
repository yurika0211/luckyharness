...
### v0.80.0: config 包覆盖率冲刺 80% (77.0%→80%+)

**状态**: Ready
**优先级**: High
**目标**: config 包 77.0% → 80%+ (+3.0pp)
**预计耗时**: 20-30 分钟

**待覆盖函数**:
- `Set`: 53.8% - 补充边界条件测试
- `Load`: 66.7% - 补充文件不存在/损坏情况
- `Save`: 70.0% - 补充保存失败情况
- `NewManager`: 75.0% - 补充错误路径
- `InitHome`: 80.0% - 补充边缘情况

**任务列表**:
- [ ] 分析 config_test.go 现有测试
- [ ] 补充 Set/Load/Save 边缘情况测试
- [ ] 运行测试验证覆盖率 → 目标 80%+
- [ ] 提交并推送代码
